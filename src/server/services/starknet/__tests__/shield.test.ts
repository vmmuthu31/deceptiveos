...
describe('shieldStrk', () => {
  ...
  it('should export shieldStrk function', async () => {
    const { shieldStrk } = await import('../shield');
    expect(typeof shieldStrk).toBe('function');
  });

  it('should throw on zero amount', async () => {
    const { shieldStrk } = await import('../shield');
    await expect(shieldStrk(0n)).rejects.toThrow('Shield amount must be positive');
  });

  it('should throw on negative amount', async () => {
    const { shieldStrk } = await import('../shield');
    await expect(shieldStrk(-100n)).rejects.toThrow('Shield amount must be positive');
  });

  it('should return correct result shape', async () => {
    const { shieldStrk } = await import('../shield');
    const result = await shieldStrk(1000n);
